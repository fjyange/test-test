...
/**
 * 接口登录 Time：2018年8月28日 下午1:55:34<br>
 * 
 * @author zhenglin
 * @version 1.0.0
 * @since JDK1.6
...
public class UserLogin
{
	...
	public ResultVO<Record<String, Object>> login(AeolusData data)
			throws ServiceException
	{
		...
		try
		{
			Record<String, Object> userRecord = activeRecordDAO.pandora()
					.SELECT_ALL_FROM(TableName.T_SYS_USER_BASE)
					.EQUAL("USER_ACCOUNT", account).EQUAL("PASSWORD", password)
					.get();
			if (CollectionUtils.isEmpty(userRecord))
			{
				logger.error(LogUtils.format("登录失败,账号不存在或密码不正确",
						new Object[] { account }));
			...
			{
				logger.error(LogUtils.format("登录失败,账号被禁用!",
						new Object[] { account }));
				resultVO.setSuccess(false);
				resultVO.setErrorDesc("登录失败,账号不存在或密码不正确");
				return resultVO;
			...
			resultVO.setResult(new RecordImpl<String, Object>().setColumn("token",
					token));
		}
