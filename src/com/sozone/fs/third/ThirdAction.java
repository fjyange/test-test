...
public class ThirdAction
{
	...
	@Service
	public synchronized ResJson sendorder(AeolusData aeolusData)
			throws FacadeException
	{
		...
		logRecord.setColumn("V_RECIEVE_CONTENT",
				aeolusData.getRecord().toString());
		...
		Record<String, Object> checkOrder = this.activeRecordDAO.pandora()
				...
				.EQUAL("V_ORDER_NO", orderno).EQUAL("V_MONEY", money)
				.EQUAL("V_BELONG_APP", appRecord.getString("ID")).get();
		if (!CollectionUtils.isEmpty(checkOrder))
		{
			Record<String, Object> accountRecord = this.activeRecordDAO
					.pandora().SELECT_ALL_FROM(Constant.TableName.T_PAY_ACCOUNT)
					.EQUAL("ID", checkOrder.getString("V_BELONG_ACCOUNT"))
					.get();
			Record<String, Object> fileRecord1 = this.activeRecordDAO.pandora()
					.SELECT_ALL_FROM(Constant.TableName.T_FILE_TAB)
					.EQUAL("ID", accountRecord.getString("V_FILE_ID")).get();
			String url1 = Constant.WEB_URL + fileRecord1.getString("V_NAME");

			String viewUrl = Constant.VIEW_URL + "/showPayApp.jsp?id="
					+ checkOrder.getString("ID");
		...
		else if (StringUtils.equals("1", payModel))
		{
			...
			try
			{
				HttpClientUtils.sendJsonPostRequest(Constant.INSER_ORDER,
		...
		this.activeRecordDAO.pandora()
				.INSERT_INTO(Constant.TableName.T_ORDER_TAB).VALUES(orderRecord)
				.excute();
		...
		this.activeRecordDAO.pandora().UPDATE(Constant.TableName.T_USER_SHOW)
				.EQUAL("V_USER_ID", fileRecord.getString("V_USER_ID"))
		...
		// 添加锁住金额
		this.activeRecordDAO.statement().update("Order.updateUserLockMoney",
				updateRecord);
		updateRecord.clear();
		...
		// 修改账户支付次数
		this.activeRecordDAO.statement().update("Order.updateAccountTimes",
				updateRecord);
		resJson.setSuccess(true);
		resJson.setMsg("下单成功");
	...
	{
		this.activeRecordDAO.pandora()
				.INSERT_INTO(Constant.TableName.T_RECIEVE_TAB).VALUES(record)
				.excute();
	}

	@Path(value = "/getOrderMsg/{id}", desc = "获取订单信息")
	@Service
	public ResJson getOrderMsg(@PathParam("id") String id,
			AeolusData aeolusData) throws Exception
	{
		ResJson resJson = new ResJson(false, "获取订单信息失败");
		Record<String, Object> params = new RecordImpl<>();
		params.setColumn("ID", id);
		Record<String, Object> orderRecord = this.activeRecordDAO.statement()
				.getOne("Order.getOrderMsg", params);
		orderRecord.setColumn("IMG_URL",
				Constant.WEB_URL + orderRecord.getString("V_NAME"));
		orderRecord.setColumn("ALI_URL",
				Constant.ALI_URL + orderRecord.getString("V_URL_SCHEME"));
		params.clear();
		if (StringUtils.equals("1", orderRecord.getString("V_VIEW_TYPE")))
		{
			if (StringUtils.isEmpty(orderRecord.getString("V_QR_CODE")))
			{
				Thread.sleep(1000);
				Record<String, Object> payRecord = new RecordImpl<String, Object>();
				payRecord.setColumn("V_RECID",
						orderRecord.getString("V_APP_ID"));
				...
				try
				{
					String result = HttpClientUtils.sendJsonPostRequest(
							Constant.QRCODE_URL,
							JSONObject.toJSONString(payRecord), "utf-8");
			...
			{
				orderRecord.setColumn("ZZ_URL",
						orderRecord.getString("V_QR_CODE"));
			}
		}
		...
		orderRecord.setColumn("DOWN_URL",
				Constant.VIEW_URL + "/authorize/attach/downFile?ID="
						+ orderRecord.getString("ID"));
	...
	@Service
	public ResJson getTestOrderMsg(@PathParam("id") String id,
			AeolusData aeolusData) throws Exception
	{
		ResJson resJson = new ResJson(false, "获取订单信息失败");
		Record<String, Object> params = new RecordImpl<>();
	...
	@Service
	public synchronized ResJson testOrder(@PathParam("model") String model,
			AeolusData aeolusData) throws FacadeException
	{
		logger.debug(LogUtils.format("接收订单信息", aeolusData));
		ResJson resJson = new ResJson(false, "下单失败");
		...
		Record<String, Object> fileRecord = this.activeRecordDAO.statement()
				.selectOne("Third.getpaytestimage", params);
		String url = Constant.WEB_URL + fileRecord.getString("V_NAME");
		resJson.setMapData("result", url);
	...
	@Service
	public void pageTime(@PathParam("id") String id,
			@PathParam("time") String time) throws Exception
	{
		Record<String, Object> record = new RecordImpl<>();
		record.setColumn("V_LONG_TIME", time);
		this.activeRecordDAO.pandora().UPDATE(Constant.TableName.T_ORDER_TAB)
				.EQUAL("ID", id).SET(record).excute();
	}

	@Path(value = "/pagetype/{id}/{type}", desc = "用户停留时间")
	@Service
	public void pagetype(@PathParam("id") String id,
	...
	@Service
	public ResJson acctest(@PathParam("appid") String appId)
			throws FacadeException
	{
	...
	@Service
	public ResJson ortest(@PathParam("appid") String appid,
			@PathParam("orderno") String order) throws Exception
	{
		...
		{
			String result = HttpClientUtils.sendJsonPostRequest(
					Constant.QRCODE_URL, JSONObject.toJSONString(payRecord),
					"utf-8");
			JSONObject jsonObject = JSONObject.parseObject(result);
			String qrcode = jsonObject.getString("V_QRCODE");
			resJson.setSuccess(true);
	...
	public static void main(String[] args) throws Exception
	{
		...
		System.out.println(HttpClientUtils.sendJsonPostRequest(
				"http://www.hanayiqi.com/authorize/third/sendorder",
