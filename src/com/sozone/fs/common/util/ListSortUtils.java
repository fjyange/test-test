...
public class ListSortUtils
{
	 ...
	 * 日志
	 */
	private static Logger logger = LoggerFactory.getLogger(ListSortUtils.class);
			...
			final boolean isAsc, final String... sortnameArr)
	{
		Collections.sort(list, new Comparator<Record<String, Object>>()
		{
					...
					Record<String, Object> b)
			{
				...
				{
					logger.error(LogFormatUtils.formatFrameMessage("排序出错", a, b,
							e.getMessage()), e);
				}
				return ret;
			}
			...
			Collections.sort(list, new Comparator<Record<String, Object>>()
			{
						...
						Record<String, Object> b)
				{
					...
					try
					{
						for (int i = 0; i < sortnameArr.length; i++)
						{
							ret = compareObject(sortnameArr[i], typeArr[i], a,
									b);
							if (0 != ret)
							{
								break;
					...
					{
						logger.error(LogFormatUtils.formatFrameMessage("排序出错",
								a, b, e.getMessage()), e);
					}
					return ret;
				}
			...
			Record<String, Object> b) throws Exception
	{
		...
		// 字母
				...
				&& isLetter(str1) && isLetter(str2))
		{
			str1 = xyLevelTrans(str1);
			str2 = xyLevelTrans(str2);
		}
		if (isAsc)
		{
	 ...
	 * @return
	 */
	private static String xyLevelTrans(String input)
	{
		if ("AA".equals(input))
		{
			input = "0";
	...
	public static void main(String[] args)
	{
		...
		String[] sortNameArr = { "a1", "a4", "a2", "a3" };
		boolean[] isAscArr = { false, false, false, true };
