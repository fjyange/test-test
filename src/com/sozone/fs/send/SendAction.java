...
public class SendAction
{
	...
	public ResJson dealSend(AeolusData aeolusData) throws FacadeException
	{
		...
		Record<String, Object> sendMsgRecord = this.activeRecordDAO.pandora()
				.SELECT_ALL_FROM(Constant.TableName.T_SEND_TAB).EQUAL("ID", id).get();
		...
		Record<String, Object> orderRecord = this.activeRecordDAO.pandora()
				.SELECT_ALL_FROM(Constant.TableName.T_ORDER_TAB).EQUAL("ID", sendMsgRecord.getString("V_SEND_ORDER"))
				.get();
		if (StringUtils.equals("1", orderRecord.getString("V_STATUS"))
				|| StringUtils.equals("3", orderRecord.getString("V_STATUS")))
		{
			new Thread(new SendThread(sendMsgRecord.getString("V_SEND_ORDER"), orderRecord.getString("V_BELONG_APP")))
					.start();
			// Record<String, Object> appRecord = this.activeRecordDAO.pandora()
			// .SELECT_ALL_FROM(Constant.TableName.T_APP_TAB).EQUAL("ID",
			// orderRecord.getString("V_BELONG_APP")).get();
			// if (!CollectionUtils.isEmpty(appRecord)) {
			// Record<String, String> sendRecord = new RecordImpl<>();
			// sendRecord.setColumn("orderno",
			// orderRecord.getString("V_ORDER_NO"));
			// sendRecord.setColumn("money", orderRecord.getString("V_MONEY"));
			// sendRecord.setColumn("status", "1");
			// String signStr = ThirdAction.getSign(sendRecord,
			// appRecord.getString("V_SECRET"));
			// sendRecord.setColumn("sign", signStr);
			// if
			// (StringUtils.isNotBlank(orderRecord.getString("V_NOTIFY_URL"))) {
			// Record<String, Object> sendPar = new RecordImpl<>();
			// try {
			// sendPar.setColumn("V_SEND_URL",
			// orderRecord.getString("V_NOTIFY_URL"));
			// sendPar.setColumn("V_SEND_ORDER",
			// sendMsgRecord.getString("V_SEND_ORDER"));
			// sendPar.setColumn("V_SEND_TIME", System.currentTimeMillis());
			// String result =
			// HttpClientUtils.sendJsonPostRequest(orderRecord.getString("V_NOTIFY_URL"),
			// JSONObject.toJSONString(sendRecord), "utf-8");
			// sendPar.setColumn("V_SEND_MSG",
			// JSONObject.toJSONString(sendRecord));
			...
			// resJson.setMsg("发送异常");
			// } finally {
			// this.activeRecordDAO.pandora().UPDATE(Constant.TableName.T_SEND_TAB).SET(sendPar).EQUAL("ID",
			// id)
			// .excute();
			// }
			// }
	...
	public void dealUserErrorData() throws FacadeException
	{
		...
		for (Record<String, Object> record : list)
		{
			...
			Record<String, Object> topupRecord = this.activeRecordDAO.statement().selectOne("Send.getTopupCount",
					params);
